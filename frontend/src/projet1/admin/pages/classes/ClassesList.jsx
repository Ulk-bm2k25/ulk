...
const ClassesList = ({ onViewDetails, onManageAffectations }) => {
  ...
  // Table: `classes`
  // Relations:
  // - niveau -> `niveaux_scolaires`
  // - studentCount -> count(`affectations_classes` where classe_id = id)
  // - capacity -> (Valeur fixe ou colonne à ajouter dans `classes`, ex: 'capacite_max')
  // - mainTeacher -> `enseignants` (titulaire)
  const mockClasses = [
    ...
    { id: 6, name: 'Tle C', level: 'Lycée', series: 'Scientifique', studentCount: 36, capacity: 35, mainTeacher: 'Pr. Zinsou' }, // Surchargé
  ];
  ...
  const getProgressColor = (current, max) => {
    ...
    if (percentage >= 100) return 'bg-red-500'; // Surchargé
    if (percentage >= 85) return 'bg-orange-500'; // Critique
  ...
  const handleSaveClass = (newClassData) => {
    console.log("Sauvegarde BDD :", newClassData);
    // Ici, plus tard : await fetch('/api/classes', { method: 'POST' ... })
    // Puis rafraichir la liste
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button 
          onClick={() => setIsModalOpen(true)} // <--- Ouverture ici
