...
const MoveStudentModal = ({ isOpen, onClose, student, onConfirm }) => {
  const [selectedClassId, setSelectedClassId] = useState('');
  const [forceOverride, setForceOverride] = useState(false); // Pour forcer si plein

  // --- MOCK DATA CLASSES (À remplacer par API) ---
  const classes = [
    { id: 1, name: '6ème A', capacity: 50, current: 45, level: 'Collège' },
    { id: 2, name: '6ème B', capacity: 50, current: 22, level: 'Collège' },
    { id: 4, name: '2nde C', capacity: 35, current: 32, level: 'Lycée' },
    { id: 6, name: 'Tle C', capacity: 35, current: 36, level: 'Lycée' }, // Déjà surchargée
  ];
  ...
  const isFull = targetClass ? targetClass.current >= targetClass.capacity : false;
  const isOverloaded = targetClass ? targetClass.current > targetClass.capacity : false;
        ...
        <div className="p-6 space-y-6">
                ...
                <div className={`p-4 rounded-lg flex items-start gap-3 ${isOverloaded ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-orange-50 text-orange-700 border border-orange-100'}`}>
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs opacity-90">
                            Cette classe compte déjà {targetClass.current} élèves pour {targetClass.capacity} places. L'ajout d'un élève dépassera le seuil autorisé.
                        </p>
            ...
            {targetClass && !isFull && (
                <div className="flex items-center gap-2 text-green-600 bg-green-50 p-3 rounded-lg border border-green-100 text-sm font-medium">
                    <CheckCircle size={16} />
                    <span>Place disponible. Capacité après ajout : {targetClass.current + 1}/{targetClass.capacity}</span>
                </div>
            )}
